...
class ReportHistoryStoreTests(unittest.TestCase):
    ...
    def test_every_database_connection_is_closed(self) -> None:
        ...
        tracked_path = Path(self.temporary_directory.name) / "tracked.sqlite3"
        with patch("ui_web.report_history.sqlite3.connect", side_effect=tracked_connect):
            store = ReportHistoryStore(tracked_path)
            store.load_records()
            store.load_sessions("company")
